...
function renderPlayers(players){
  $('#playersGrid').innerHTML = players.map(player => `
    <div class="player">
      <div class="mini-crest">${initials(player.Jogador)}</div>
...
function renderNextMatch(players, matches){
  ...
  $('#nextMatchBox').innerHTML = `
    <div class="team"><div class="crest">${initials(next['Jogador 1'])}</div><div class="team-name">${next['Jogador 1']}</div><p class="muted">${p1['Seleção'] || ''}</p></div>
    <div class="versus-mark">×</div>
    <div class="team"><div class="crest">${initials(next['Jogador 2'])}</div><div class="team-name">${next['Jogador 2']}</div><p class="muted">${p2['Seleção'] || ''}</p></div>
  `;
}
...
function renderLatestResult(players, matches){
  ...
  $('#latestResultBox').innerHTML = `
    <div class="score-line">
      <div class="mini-crest">${initials(finished['Jogador 1'])}</div>
      <strong>${finished['Gols 1']} - ${finished['Gols 2']}</strong>
      <div class="mini-crest">${initials(finished['Jogador 2'])}</div>
    </div>
  ...
  $('#fixturesList').innerHTML = matches.map(m => {
    ...
    return `
      <div class="match-row">
        ...
        <div class="match-team"><span class="mini-crest">${initials(m['Jogador 1'])}</span><span>${m['Jogador 1']} <small>(${p1['Seleção'] || '-'})</small></span></div>
        <div class="versus-mark">${middle}</div>
        <div class="match-team"><span class="mini-crest">${initials(m['Jogador 2'])}</span><span>${m['Jogador 2']} <small>(${p2['Seleção'] || '-'})</small></span></div>
