...
const FLAG_BY_COUNTRY = {
  'brasil':'🇧🇷','suecia':'🇸🇪','croacia':'🇭🇷','escocia':'🏴','pais de gales':'🏴','polonia':'🇵🇱','argentina':'🇦🇷','franca':'🇫🇷','portugal':'🇵🇹','espanha':'🇪🇸','alemanha':'🇩🇪','italia':'🇮🇹','inglaterra':'🏴','holanda':'🇳🇱','paises baixos':'🇳🇱','belgica':'🇧🇪','uruguai':'🇺🇾','colombia':'🇨🇴','chile':'🇨🇱','mexico':'🇲🇽','estados unidos':'🇺🇸','eua':'🇺🇸','japao':'🇯🇵','coreia do sul':'🇰🇷','marrocos':'🇲🇦','senegal':'🇸🇳','gana':'🇬🇭','costa do marfim':'🇨🇮','camaroes':'🇨🇲','nigeria':'🇳🇬','egito':'🇪🇬','turquia':'🇹🇷','dinamarca':'🇩🇰','noruega':'🇳🇴','suica':'🇨🇭','austria':'🇦🇹','servia':'🇷🇸','ucrania':'🇺🇦','russia':'🇷🇺','canada':'🇨🇦','australia':'🇦🇺','arabia saudita':'🇸🇦','qatar':'🇶🇦','iran':'🇮🇷','equador':'🇪🇨','peru':'🇵🇪','paraguai':'🇵🇾','bolivia':'🇧🇴','venezuela':'🇻🇪','costa rica':'🇨🇷'
};
function flagEmoji(country){
  return FLAG_BY_COUNTRY[normalizeCountry(country)] || '⚑';
}
function flagCircle(country, extraClass = ''){
  const safe = String(country || 'Sem seleção').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');
  return `<span class="flag-circle ${extraClass}" title="${safe}" aria-label="${safe}">${flagEmoji(country)}</span>`;
}
